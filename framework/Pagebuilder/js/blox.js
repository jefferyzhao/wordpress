...
function add_event_blox_elements(){
    ...
    setTimeout(function(){
        jQuery("#blox_contentbuilder").bind("DOMNodeInserted", function(objEvent){
            if( jQuery(objEvent.target).hasClass('blox_item') ){
                setTimeout(function(){
                    updating_tinymce_content();
                }, 500)
            }
        ...
        jQuery("#blox_contentbuilder").bind("DOMNodeRemoved", function(objEvent){
            if( jQuery(objEvent.target).hasClass('blox_item') ){
                setTimeout(function(){
                    updating_tinymce_content();
                }, 500)
...
function updating_tinymce_content(){
    ...
    var editor = blox_get_tinymce_editor();
	if ( editor && ! editor.isHidden() ){
		window.tinyMCE.get('content').setContent( window.switchEditors.wpautop($content.html()), {format:'raw'} );
	}
	else{
		jQuery('#content.wp-editor-area').val( window.switchEditors.wpautop($content.html()) );
	}
	...
	setTimeout(function(){
		if ( editor && ! editor.isHidden() ){
			...
			if( typeof switchEditors.switchto !== 'undefined' ){
				switchEditors.switchto(document.getElementById('content-tmce'));
			}
			else{
				switchEditors.go(false, 'tmce');
			}
		}
	}, 1000);
}
...
function blox_get_tinymce_editor(){
...
function switch_blox_builder(switcher){
	...
	if(switcher){
		...
		var $content = jQuery('#content.wp-editor-area').val();
		if ( editor && ! editor.isHidden() ) {
